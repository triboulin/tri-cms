...
CREATE TABLE webhooks (
    ...
    events TEXT NOT NULL, -- Liste JSON des évènements (ex: ["content.update", "media.create"])
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE webhook_deliveries (
    ...
    deploy_resolved_at DATETIME,                   -- stamped once, the moment deploy_status first becomes 'completed'
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
```
...
> - `global_logs.action` suit une convention `<ressource>.<verbe>` (ex : `project.create`, `user.suspend`). `webhooks.events` suivait historiquement la même granularité mais a été volontairement simplifié (voir point suivant) : `global_logs.action` garde le détail du verbe CRUD (`content.create`/`update`/`delete`/`publish`/`unpublish`) pour la piste d'audit, là où la distinction reste utile ; seule la souscription webhook est passée à un évènement unique.
> - `webhooks.events` : **un seul évènement de contenu, `content.update`, couvre désormais toute forme de mutation CRUD** (création, édition, suppression, publication, dépublication) — il n'y a plus d'évènements `content.create`/`content.delete`/`content.publish`/`content.unpublish` distincts à choisir. Ce choix élimine une classe entière de bug par construction : un webhook souscrit à un évènement granulaire que le récepteur (ex : le `types:` d'un workflow GitHub Actions) n'écoute pas réellement envoie indéfiniment des livraisons « réussies » sans jamais rien déclencher côté récepteur — c'est exactement ce qui s'est produit sur `neon-blog` (voir `spec_sveltekit_tricms_cloudflare.md` §7.1, note « Piège vécu »). Les seuls autres évènements existants sont `media.create` et `media.delete`, qui restent distincts. Comportement par défaut inchangé : si le formulaire IHM ou l'appel API `POST/PUT .../webhooks` omet `events` (liste vide), triCMS souscrit automatiquement le webhook à la totalité des évènements connus (`content.update`, `media.create`, `media.delete`) plutôt que de rejeter la requête ou de ne rien souscrire. Le formulaire de création (`web/templates/pages/webhooks.html`) reflète ce défaut : les cases à cocher y sont toutes précochées.
...
> - `webhook_deliveries.success` ne reflète que l'appel HTTP de triCMS lui-même (POST générique accepté, ou `repository_dispatch` accepté par l'API GitHub) — pas le résultat du build/déploiement déclenché en aval. Pour les webhooks `kind=github_dispatch`, les colonnes `deploy_*` suivent ce résultat réel via **deux boucles de polling découplées, la base de données comme unique intermédiaire entre elles** :
>   - **Côté Go (écriture)** : `Server.RunDeployStatusPoller` (`pkg/api/deploy_poller.go`), une goroutine lancée au démarrage (`cmd/tricms/main.go`) et indépendante de toute requête HTTP, interroge toutes les 5 s (`DeployStatusPollInterval`) `storage.ListPendingGitHubDispatchDeliveries` (toutes les livraisons `kind=github_dispatch` réussies et non résolues, tous projets confondus, plafonnées à `DeployStatusMaxAge` = 30 min pour ne pas réessayer indéfiniment un run introuvable). Pour chacune : corrélation au run GitHub Actions déclenché (`pkg/webhooks.Dispatcher.FindDispatchRun`, par proximité temporelle avec `webhook_deliveries.created_at` — l'API `repository_dispatch` ne renvoie aucun identifiant de run), puis interrogation de son statut (`Dispatcher.GetRun`), et persistance via `UpdateWebhookDeliveryDeployState` (qui horodate aussi `deploy_resolved_at` au premier passage à `completed`). Toute erreur (réseau, ou PAT sans la permission **Actions: Read**, distincte de **Contents: Read and write** déjà requise pour l'envoi) est journalisée et absorbée, réessayée au tick suivant.
>   - **Côté front (lecture)** : `htmxWebhooks` et `htmxDeployTile` ne font jamais d'appel à l'API GitHub — ils lisent uniquement ce que la boucle Go a déjà persisté. La page Webhooks affiche l'historique complet (rafraîchissement HTMX `every 5s`, existant) ; une tuile pleine largeur en haut de **chaque page d'un projet** (`partial:topbar` → `#deploy-tile`, `hx-trigger="load, every 5s"`) affiche l'état de la dernière livraison `github_dispatch` réussie (`storage.LatestGitHubDispatchDelivery`) tant qu'elle n'est pas dans un état final, puis pendant encore 30 s après (`deployTileGracePeriod`, `pkg/api/htmx_deploy_tile.go`) avant de disparaître. Accessible à tout membre du projet (`loadProjectForHTMXAny`), pas seulement au rôle ADMIN requis pour gérer les webhooks eux-mêmes.
>   - Ce découplage garantit qu'une API GitHub lente ou indisponible ne bloque jamais une page (le front ne l'appelle jamais directement), et que le suivi continue même si personne n'a de page triCMS ouverte au moment où le build se termine.
