...
> - `webhooks.events` : **comportement par défaut attendu = tous les évènements possibles.** Si le formulaire IHM ou l'appel API `POST/PUT .../webhooks` omet `events` (liste vide), triCMS souscrit automatiquement le webhook à la totalité des types d'évènement connus (`content.create`, `content.update`, `content.delete`, `content.publish`, `content.unpublish`, `media.create`, `media.delete`) plutôt que de rejeter la requête ou de ne rien souscrire. Le formulaire de création (`web/templates/pages/webhooks.html`) reflète ce défaut : les cases à cocher y sont toutes précochées. Restreindre à un sous-ensemble reste possible et volontaire (voir par ex. le webhook `github_dispatch` de rebuild, qui ne souscrit délibérément qu'à `content.publish`/`content.unpublish` — `spec_sveltekit_tricms_cloudflare.md` §8.1).
